...
export async function generateStaticParams() {
  const data: PokemonsResponse = await fetch('https://pokeapi.co/api/v2/pokemon?limit=151').then((resp) =>
    resp.json()
  );
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  try {
    ...
    return { title: pokemon.name, description: `Página del pokémon ${pokemon.name}` };
  } catch (error) {
    return {
